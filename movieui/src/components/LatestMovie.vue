<template>
  ...
  <v-container v-else grid-list-xl fluid>
    ...
    <v-layout wrap row >
      <v-flex :[dynamicFlex]="true" v-for="(item, index) in wholeResponse" :key="index">
        <v-card   height=100% >
          <image-overview
          ...
          ></image-overview>
          <div v-if= !mobile>
            <burger-menu v-if="size > 2" :movie="item"></burger-menu>
            <div>
              <div >
                <div
                  v-if="size > 2"
                  @click="openModal(item)"
                  v-bind:class="{ moviesmall: size < 4}"
                  style="padding-top: 15px; overflow-y: hidden; padding-left: 10px;"
                >
                  <span
                    class="movietitle"
                    ...
                    v-html="title(item)"
                  >{{title(item)}}</span>
                  <span class="release">{{release(item)}}</span>
                </div>
                <div class="moviedesc">
                  <div v-if="item.meta">{{item.meta.Overview | truncate(descLength()) }}</div>
                  <div class="movieaction">
                    <div v-if="multipleChoice(item)">
                      <v-btn outline color="primary" dark @click="openChoiceModal(item)">
                        Find
